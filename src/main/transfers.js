...
// Pliki mają postęp bajtowy; katalogi (rekurencyjnie) lecą jako nieokreślony postęp.
  ...
  async _run() {
    ...
    while (this.queue.length) {
      ...
      try {
        ...
        if (task.direction === 'upload') {
          if (task.isDir) await session.uploadDir(task.localPath, task.remotePath, onProgress)
          else await session.uploadFile(task.localPath, task.remotePath, onProgress)
        } else {
          if (task.isDir) await session.downloadDir(task.remotePath, task.localPath, onProgress)
          else await session.downloadFile(task.remotePath, task.localPath, onProgress)
        }
